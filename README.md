...
Records the start timestamp (intentionally before service restart, to capture the full shutdown/startup lifecycle)
Restarts SecureAiService (net stop then net start) to exercise the registration sequence and ETW session startup
Launches the httpbin fixture as a background process and patches roster.json with the live PID
Launches the chatgpt fixture for DNS/TCP correlation testing
Patches roster.json with the live httpbin PID
Launches the python+torch fixture as a background process for library detection and module enumeration tests, and patches roster.json with the live PID
Launches the OpenAI curl fixture for TCP connect testing and patches roster.json with the live PID
Launches the Anthropic fixture for IPv6 TCP testing
Waits for Docker to be ready, then pulls required images (ollama/ollama, nginx, python:3.11, n8nio/n8n)
Launches five Docker containers: ollama_mount_test (with model volume), nginx_test, langchain_test, n8n_test, pyai_test
Waits 240 seconds for scanner poll cycles, registration attempts, DNS cache refresh, and container detection
Runs two Schannel fixtures via PowerShell (copilot.microsoft.com, chat.openai.com)
Waits 20 seconds for TLS events to appear in the log
Invokes overall.py with the recorded start timestamp
Terminates all fixtures and cleans up Docker containers
...
Launch httpbin fixture and note the PID:
...
Launch python+torch fixture and note the PID:
python -c "import torch, time, os; print(f'PID: {os.getpid()}', flush=True); time.sleep(300)"
Launch curl fixture and note the PID:
curl https://openai.com
Update roster.json — SAVR14.by_pid with the httpbin PID, SAVR12.by_pid with the curl PID, and SAVR17.expected_agents[0].pid with the python PID
...
Run Schannel fixtures in PowerShell:
powershell
...
Output Files
results.csv — one row per assertion, with columns test, subject, expected, actual, result, comments. This is the primary output; see Reading the Results for verdict meanings.
