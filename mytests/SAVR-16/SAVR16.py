...
Harness contract: name, __init__(cfg), offer(line, i, window),
resolve(), rows(). Enable via a "kernel_file_monitor_test" section
in roster.json.
"""
...
# stats block lines, e.g.  "[ETW_FILE_MONITOR] Events Received(12) : 0"
_STAT_RE = re.compile(
...
class KernelFileMonitorTest:
    ...
    def __init__(self, cfg):
        ...
        self.max_cpu = cfg.get("max_cpu_pct")          # None = skip check
        self.max_mem_kb = cfg.get("max_memory_kb")     # None = skip check
        self.matched_min = int(cfg.get("expect_matched_min", 0))
        self.dropped_min = int(cfg.get("expect_dropped_min", 0))

        self.session_line = None       # (i, line) first sighting
        self.started = None            # dict from _STARTED_RE + timestamp
        self.stopped_idx = None
        self.svc_stop_idx = None
        self.stats = {}                # latest value per counter key
        self.stats_ts = None           # timestamp of the latest stats block
        self.cpu_pct = None
        self.mem_kb = None
    ...
    def offer(self, line, i, window):
        ...
        if m:
            key = m.group("key")
            # normalize "Events Received(12)" -> "Events Received"
    ...
    def resolve(self):
        ...
        # nothing at all -> single NOT_DETECTED row and stop
